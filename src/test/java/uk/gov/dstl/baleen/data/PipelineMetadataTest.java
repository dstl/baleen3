...
public class PipelineMetadataTest {
  @Test
  public void test(){
    PipelineHolder holder = mock(PipelineHolder.class);
    PipelineDescriptor descriptor = mock(PipelineDescriptor.class);

    when(descriptor.getName()).thenReturn("NAME");
    when(descriptor.getDescription()).thenReturn("DESC");
    when(holder.getDescriptor()).thenReturn(descriptor);

    PipelineMetadata metadata = new PipelineMetadata(holder);
